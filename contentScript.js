...
(function() {
    const memoriesSVG = `
      <svg fill="#40414f" height="16px" width="16px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 612.001 612.001" xml:space="preserve" stroke="#40414f">
    ...
    const styles = `
        ...
        .dark [data-test-render-count] .memory-section {
    ...
    const createMemoriesIcon = () => {
        ...
        return parser.parseFromString(memoriesSVG, 'image/svg+xml').documentElement;
    };
    ...
    const insertMemories = (memories) => {
        ...
        const existingText = inputBox.innerText;
        ...
        let newText;
        if (existingText.includes('<relevant_past_memories>')) {
            newText = existingText.replace(/<relevant_past_memories>[\s\S]*?<\/relevant_past_memories>/, memoriesTag);
        } else {
    ...
    const handleMessageStyling = (messageContainer) => {
        ...
        const match = messageDiv.textContent.match(/<relevant_past_memories>([\s\S]*?)<\/relevant_past_memories>/);
        if (!match) return;

        const [fullMatch, memoriesContent] = match;
        const [before, after] = messageDiv.textContent.split(/<\/?relevant_past_memories>/);
        
        messageDiv.innerHTML = `
            ${before}
            <div class="memory-section">
                ${createMemoriesIcon().outerHTML} ${memoriesContent}
            </div>
            ${after || ''}
        `;
    };
    ...
    const getAndInsertMemories = async (button) => {
        try {
            ...
            const existingText = inputBox.innerText;
            const cleanedText = existingText.replace(/<relevant_past_memories>[\s\S]*?<\/relevant_past_memories>/, '').trim();

            if (!cleanedText) {
                console.warn('No input text found after removing <relevant_past_memories> section.');
                return;
            }
    ...
    const addGetMemoriesButton = () => {
        ...
        const target = 
            document.querySelector('fieldset .flex.flex-col.bg-bg-000') ||
    ...
    const createMemoriesButton = () => {
        ...
        button.innerHTML = `${memoriesSVG}<span>Get Memories</span>`;
